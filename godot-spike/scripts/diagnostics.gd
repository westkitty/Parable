...
func snapshot_lines() -> Array[String]:
	...
	if hand:
		...
		lines.append("hold anchor: %s" % str(d.get("hold_anchor", Vector3.ZERO)))
		...
		lines.append("hold anchor local: %s grip-anchor dist %.2f" % [str(d.get("hold_offset", Vector3.ZERO)), d.get("hold_distance", 0.0)])
		lines.append("hold debug markers: %s" % str(d.get("hold_debug_markers", false)))
