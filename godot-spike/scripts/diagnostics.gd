...
func snapshot_lines() -> Array[String]:
	...
	if hand:
		...
		lines.append("palm center: %s" % str(d.get("palm_center", Vector3.ZERO)))
		lines.append("hand grip contact: %s" % str(d.get("grip_point", Vector3.ZERO)))
		lines.append("object grip contact: %s" % str(d.get("grip_contact", Vector3.ZERO)))
		lines.append("held object position: %s" % str(d.get("held_position", Vector3.ZERO)))
		lines.append("hold profile: %s hand pose: %s" % [d.get("hold_profile", "-"), d.get("hand_pose", "-")])
		lines.append("grip contact local: %s grip-contact dist %.2f" % [str(d.get("hold_offset", Vector3.ZERO)), d.get("hold_distance", 0.0)])
		lines.append("hold debug markers active: %s" % str(d.get("hold_debug_markers", false)))
